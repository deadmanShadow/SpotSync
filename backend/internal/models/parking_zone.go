...
type ParkingZone struct {
	...
	// available_spots = total_capacity - (count of 1s in SpotHolds)
	// - active_reservations - rotation_hold, so held spots are additive
	// to real reservations and "truly off-limits" for booking.
	//
	// IMPORTANT: the held count is the number of array elements equal to
	// 1, NOT len(SpotHolds). len() always returns TotalCapacity for a
	// properly-sized bitmap, which would make every zone look full.
	//
